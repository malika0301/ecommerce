...
let newsCards = document.getElementById("new");
let newFoursProducts = products.slice(products.length - 4, products.length);
...
function showSingleProducts(){
    ...
    item.map((el) => {
        singleCards.innerHTML += `
    <div>
                <h4 class="font-bold text-[50px]">${el.description}</h4>
    </div>
    ...
    <div class="flex gap-[20px]">
          ...
          <span class="bg-white p-[10px] flex items-center justify-center text-[18px] font-bold">${carts.find((item) => item.id === el.id).numbers}</span>
          <button
...
showProduct(newsCards, newFoursProducts);